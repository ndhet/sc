...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/ndhet/izinvps/ipuk/ip > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/ndhet/izinvps/ipuk/ip | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/ndhet/izinvps/ipuk/ip | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
Exp=$(curl -sS https://raw.githubusercontent.com/ndhet/izinvps/ipuk/ip | grep $MYIP | awk '{print $3}')
fi
clear
MYIP=$(curl -sS ipv4.icanhazip.com)
#########################
